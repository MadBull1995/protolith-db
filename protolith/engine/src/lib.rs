...
use protolith_core::{
    api::{
        get_protolith_file_descriptor,
        protolith::{
            self,
            core::v1::{Database},
            services::v1::{CreateDatabaseResponse, ListDatabasesResponse},
...
pub trait Engine: Metadata + Sync + Send + 'static {
    fn list_databases(
        &self,
...
impl Engine for ProtolithDbEngine {
    ...
    async fn insert(
        ...
        match db {
            ...
            Some(db) => { 
                let rep = db.insert(message).map_err(|err| EngineError::OpError(OpError::KeyAlreadyExists(err)));
                rep
            }
        }
    }
    ...
    async fn create_database(&self, name: String, fd_descriptor: Vec<u8>) -> Result<CreateDatabaseResponse, EngineError> {
        ...
        if inner.dbs.contains_key(&name) {
            ...
            let descriptor_path = self.db_config.db_path.join(name.clone()).join("descriptor.bin");
        ...
        if let Some(db) = inner.dbs.remove(db_name) {
            ...
            match DB::destroy(&Options::default(), &db_path) {
                Ok(_) => {
                    ...
                    match removed_file {
                        Err(err) => {
                            error!(file = ?pb_descriptor_file, error = ?err, "Failed to remove protobuf descriptor file")
                        },
                        Ok(_) => debug!(file = ?pb_descriptor_file, "Removed protobuf descriptor file")
                    }
                    Ok(())
                },
